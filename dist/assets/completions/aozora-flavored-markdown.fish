...
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_needs_command" -f -a "fmt" -d 'Canonicalize source and check, display, or write the result'
...
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -l encoding -d 'Input encoding. Defaults to UTF-8; use `sjis` for raw Aozora Bunko files' -r -f -a "utf8\t''
sjis\t''"
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -l color -d 'When to colorize diagnostics: auto (TTY-aware), always, or never' -r -f -a "auto\t''
always\t''
never\t''"
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -l format -d 'Diagnostic output format: human-readable lines, or stable JSON for tooling' -r -f -a "human\t'Graphical diagnostics (severity, code, message, source snippet) for humans'
json\t'A stable `aozora-md.diagnostics.v1` JSON envelope for tooling'"
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -l check -d 'Exit 1 when the source is not canonical; write nothing'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -l diff -d 'Print a unified diff; exit 1 when the source is not canonical'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -l write -d 'Replace the input file with canonical source'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -l strict -d 'Treat any lexer/parser diagnostic as a hard error (exit 2). Default: warn and pass through'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -s v -l verbose -d 'Increase log verbosity (-v info, -vv debug, -vvv trace). `RUST_LOG` overrides'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -s q -l quiet -d 'Decrease log verbosity (-q errors only). `RUST_LOG` overrides'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand fmt" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand help; and not __fish_seen_subcommand_from render check fmt completions _man help" -f -a "render" -d 'Render the input to HTML on stdout'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand help; and not __fish_seen_subcommand_from render check fmt completions _man help" -f -a "check" -d 'Parse the input and report diagnostics without rendering'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand help; and not __fish_seen_subcommand_from render check fmt completions _man help" -f -a "fmt" -d 'Canonicalize source and check, display, or write the result'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand help; and not __fish_seen_subcommand_from render check fmt completions _man help" -f -a "completions" -d 'Generate a shell completion script on stdout'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand help; and not __fish_seen_subcommand_from render check fmt completions _man help" -f -a "_man" -d 'Render the man page (roff) on stdout. Hidden; used by packaging'
complete -c aozora-flavored-markdown -n "__fish_aozora_flavored_markdown_using_subcommand help; and not __fish_seen_subcommand_from render check fmt completions _man help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
